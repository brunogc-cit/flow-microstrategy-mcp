...
* **BREAKING:** All environment variables renamed from `NEO4J_*` to `FLOW_*`:
  - `NEO4J_URI` → `FLOW_URI`
  - `NEO4J_USERNAME` → `FLOW_USERNAME`
  - `NEO4J_PASSWORD` → `FLOW_PASSWORD`
  - `NEO4J_DATABASE` → `FLOW_DATABASE`
  - `NEO4J_READ_ONLY` → `FLOW_READ_ONLY`
  - `NEO4J_TELEMETRY` → `FLOW_TELEMETRY`
  - `NEO4J_LOG_LEVEL` → `FLOW_LOG_LEVEL`
  - `NEO4J_LOG_FORMAT` → `FLOW_LOG_FORMAT`
  - `NEO4J_SCHEMA_SAMPLE_SIZE` → `FLOW_SCHEMA_SAMPLE_SIZE`
  - `NEO4J_MCP_TRANSPORT` → `FLOW_MCP_TRANSPORT`
  - `NEO4J_MCP_HTTP_*` → `FLOW_MCP_HTTP_*`
* **BREAKING:** All CLI flags renamed from `--neo4j-*` to `--flow-*`:
  - `--neo4j-uri` → `--flow-uri`
  - `--neo4j-username` → `--flow-username`
  - `--neo4j-password` → `--flow-password`
  - `--neo4j-database` → `--flow-database`
  - `--neo4j-read-only` → `--flow-read-only`
  - `--neo4j-telemetry` → `--flow-telemetry`
  - `--neo4j-transport-mode` → `--flow-transport-mode`
  - `--neo4j-http-*` → `--flow-http-*`
